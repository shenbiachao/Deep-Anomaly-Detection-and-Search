...
# from src.models.module.deepSAD.base.base_dataset import BaseADDataset
from src.models.module.deepSAD.networks.main import build_network, build_autoencoder
from src.models.module.deepSAD.optim.DeepSAD_trainer import DeepSADTrainer
from src.models.module.deepSAD.optim.ae_trainer import AETrainer
...
class DeepSAD(object):
    ...
    def train(self, train_dataset, config):
        ...
        use_hsc = config['TRAIN']['use_hsc']
        optimizer_name = config['TRAIN']['optimizer_name']
        n_jobs_dataloader =  config['TRAIN']['n_jobs_dataloader']
        pretrain =  config['PRETRAIN']['pretrain']

        if pretrain:
            self.pretrain(train_dataset, config)
        ...
        self.trainer = DeepSADTrainer(self.c, self.eta, optimizer_name=optimizer_name, lr=lr, n_epochs=n_epochs,
                                      ...
                                      device=device, n_jobs_dataloader=n_jobs_dataloader, use_hsc=use_hsc)
        # Get the model
        self.net = self.trainer.train(train_dataset, self.net)
    ...
    def evaluate(self, test_dataset, device: str = 'cuda', n_jobs_dataloader: int = 0):
        ...
        auc_score, auc_pr = self.trainer.test(test_dataset, self.net)

        return auc_score, auc_pr
    # def pretrain(self, train_df, val_df, optimizer_name: str = 'adam', lr: float = 0.001, n_epochs: int = 100,
    #              lr_milestones: tuple = (), batch_size: int = 128, weight_decay: float = 1e-6, device: str = 'cuda',
    #              n_jobs_dataloader: int = 0):

    def pretrain(self, train_dataset, config):
        """Pretrains the weights for the Deep SAD network phi via autoencoder."""

        ## Unpack hyperparameters
        lr = config['PRETRAIN']['ae_lr']
        n_epochs = config['PRETRAIN']['ae_n_epochs']
        lr_milestones = ()
        batch_size = config['PRETRAIN']['ae_batch_size']
        weight_decay = config['PRETRAIN']['ae_weight_decay']
        device = config['PRETRAIN']['device']
        optimizer_name = config['PRETRAIN']['optimizer_name']
        n_jobs_dataloader =  config['PRETRAIN']['n_jobs_dataloader']
        ...
        # Evaliation on validation set
        # self.ae_trainer.test(val_dataset, self.ae_net)
