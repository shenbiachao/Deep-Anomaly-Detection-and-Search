...
class SAC(Base_Agent):
    ...
    def __init__(self, config, environment):
        ...
        assert self.hyperparameters["Actor"]["final_layer_activation"] != "Softmax", "Final actor layer must not be softmax"
        ...
        if self.automatic_entropy_tuning:
            self.target_entropy = -torch.prod(torch.Tensor(self.environment.action_space.shape).to(self.hyperparameters["device"])).item() # heuristic value from the paper
