...
class Environment():
    def __init__(self, dataset_a, dataset_u, parameter):
        ...
        self.clf = IsolationForest(contamination=parameter["contamination_rate"])
    ...
    def reset(self):
        ...
        self.clf = IsolationForest(contamination=self.parameter["contamination_rate"])
        self.mapped = torch.tensor([])
    ...
    def sample_method_two(self, action):
        self.current_class = 1
        candidate = np.random.choice([i for i in range(len(self.dataset_u))], size=self.parameter["sample_num"], replace=False)
        with torch.no_grad():
    ...
    def step(self, action):
        ...
        choice = np.random.choice([0, 1], size=1, p=[self.parameter["p"], 1 - self.parameter["p"]])
        if choice == 0:
